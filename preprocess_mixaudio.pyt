#!/usr/bin/env python
"""
Preprocess Mixaudio stems into 10 instrument-category stems.

For every JSON metadata file inside
/data1/dropbox/neutune's shared workspace/mixaudio_package_data/[tagging]/tagged/all
this script does the following:
 1. Locate the corresponding directory with stems under
    /data1/dropbox/neutune's shared workspace/mixaudio_package_data/[tagging]/mixaudio_package_data_stems/<json_name>/Stems
 ...
 5. Write each mixed stem into
    /data4/sake/mixaudio_10insts/<json_name>/__10_inst_files__/<category or mixture>.wav
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("preprocess_mixaudio.log", mode="a", encoding="utf-8"),
    ],
)
...
warnings_handler = logging.FileHandler("preprocess_mixaudio_warnings.log", mode="a", encoding="utf-8")
...
JSON_DIR = Path("/data1/dropbox/neutune's shared workspace/mixaudio_package_data/[tagging]/tagged/all")
TRACKS_DIR = Path("/data1/dropbox/neutune's shared workspace/mixaudio_package_data/[tagging]/mixaudio_package_data_stems")
OUTPUT_ROOT = Path("/data4/sake/mixaudio_10insts")
...
def safe_stem_path(stems_dir: Path, trackname: str) -> Optional[Path]:
    ...
    stems_dir = Path(nfc(str(stems_dir)))
    trackname = nfc(trackname)
